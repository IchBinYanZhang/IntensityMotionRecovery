function [f,phi,e] = IntensityRecovery(x,y,pol,time,triggers)
...
f = 0.5+zeros(option.ny,option.nx,option.nt+1);
...
%}
option.lambda_d = 0.0001;
option.lambda_phi = 0.0001;
option.lambda_t = 1;
option.lambda_f = 0.05;
option.solver = 'jacobian';
option.max_iter_outer1 = 500;
option.max_iter_inner1 = 1;
option.max_iter_outer2 = 500;
option.max_iter_inner2 = 1;

%% main-loop
fprintf('-derivative estimate\n');
% temporal gradient spline interpolation
%    f = IntensityEstimate(e,f,u,v,option);
phi = TemporalDerivativeEstimate_CouplingSpline(e,phi,option);
fprintf('\n');
fprintf('-image estimate\n');
f = IntensityEstimate_RobustGradientDomain(f,phi,option);
fprintf('\n');
...
function phi = TemporalDerivativeEstimate_CouplingSpline(e,phi,option)
...
switch option.solver
    case 'jacobian'
       ...
       for t = 1:option.max_iter_outer1
       ...
       [~,~,phi_t] = gradient(phim);
       psi_prime_d = 1./sqrt(1 + (phim-em).^2./lambda_d^2); %%%% on the grid (i,j,k)
       psi_prime_phi = 1./sqrt(1 + (phi_t).^2./lambda_phi^2); %%%% on the grid (i,j,k)


       %%% loop inner-2
       for tt = 1:option.max_iter_inner1
           data_term_phi = c.*psi_prime_d.*em;
           W_phi_ccp = alpha*( 0.5*psi_prime_phi + 0.5*circshift(psi_prime_phi,[0,0,-1]));
           W_phi_ccn = alpha*( 0.5*psi_prime_phi + 0.5*circshift(psi_prime_phi,[0,0,1]));

           phim = (data_term_phi + W_phi_ccp.*circshift(phim,[0,0,-1]) + W_phi_ccn.*circshift(phim,[0,0,1]) )./(W_phi_ccp+W_phi_ccn+c.*psi_prime_d);
           
           phi = phim(2:end-1,2:end-1,2:end-1);
       end
       
       fprintf('--iteration = %i\t\t min(phi) = %f\t\t max(phi) = %f\r',t,min(phi(:)),max(phi(:)));
...
function f = IntensityEstimate_RobustGradientDomain(f,phi,option)
...
switch option.solver
    case 'jacobian'
       %%% first, we unify the grid location by central averaging
       phim = MirrorImageBoundary(phi,3);
       phi_a = 0.5*phim + 0.5*circshift(phim,[0,0,-1]);
       phi_a = phi_a(:,:,1:end-1);
       phi_am = MirrorImageBoundary(phi_a);

       %%% main loop here
       for t = 1:option.max_iter_outer2
       ...
       %%% update non-linearity
       [fx,fy,ft] = gradient(fm);
       psi_prime_t = 1./sqrt(1 + (ft-phi_am).^2./lambda_t^2); %%%% this is on the grid (i,j,k)
       psi_prime_f = 1./sqrt(1 + (fx.^2+fy.^2)./lambda_f^2); 
       ...
       for tt = 1:option.max_iter_inner2
           ...
           W_f_pcc = beta*( 0.5*circshift(psi_prime_f,[0,-1,0])+ 0.5*psi_prime_f);
           W_f_ncc = beta*( 0.5*circshift(psi_prime_f,[0,1,0])+ 0.5*psi_prime_f);
           W_f_cpc = beta*( 0.5*circshift(psi_prime_f,[-1,0,0])+ 0.5*psi_prime_f);
           W_f_cnc = beta*( 0.5*circshift(psi_prime_f,[1,0,0])+ 0.5*psi_prime_f);
           W_f_ccp =  (0.5*psi_prime_t);
           W_f_ccn = -(0.5*psi_prime_t);
           data_term_f = -psi_prime_t.*phi_am; 
           cc = W_f_pcc + W_f_ncc + W_f_cpc + W_f_cnc; 
           fm = (data_term_f ...
                + W_f_pcc.*circshift(fm,[0,-1,0]) ...
                + W_f_ncc.*circshift(fm,[0,1,0]) ...
                + W_f_cpc.*circshift(fm,[-1,0,0]) ...
       ...
       fprintf('--iteration = %i\t\t min(f) = %f\t\t max(f) = %f\r',t,min(f(:)),max(f(:)));
       end
...
function um = MirrorImageBoundary(u,dim)
[ny,nx,nt] = size(u);
