...
fprintf('Reading at most %u events from file %s\n', maxevents, filename);

f=fopen([path,filename],'r');
% skip header lines
bof=ftell(f);
line=fgets(f);  % line=native2unicode(fgets(f));
tok='#!AER-DAT';
version=0;
...
%    fprintf('%s',line); % print line using \n for newline, discarding CRLF written by java under windows
    ...
    line=fgets(f);  % line=native2unicode(fgets(f)); % gets the line including line ending chars
end
